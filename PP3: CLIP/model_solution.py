"""
CLIP (Contrastive Language-Image Pre-Training) model architecture — SOLUTION.

This file contains the complete, working implementation. Students should refer
to this only after attempting model.py on their own.
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        x = torch.cat(
            ...
            dim=1,
        )  # shape = [batch_size, grid ** 2 + 1, width]
        x = x + self.positional_embedding.to(x.dtype)
        x = self.ln_pre(x)

        x = x.permute(1, 0, 2)  # [batch_size, grid ** 2 + 1, width] -> [grid ** 2 + 1, batch_size, width]
